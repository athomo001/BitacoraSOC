...
**Actualización 2026-09-23**: el legacy ya no convive en esta carpeta. Se movió a `../BitacoraSOC-legacy/` (worktree del tag `legacy-v1-final`), y el stack nuevo pasó a llamarse `docker-compose.yml` + `Caddyfile` a secas (antes `.rewrite`), con su propio `.env` (ver `.env.example`). El proyecto Compose sigue llamándose `bitacora-rewrite` para no dejar huérfano el volumen de datos existente.
