...
| UI-REPORT-GEN-140 | En progreso | UX/UI + Frontend CRÍTICA | Reinventar UI del "Generador de Reportes SOC" con nuevo diseño moderno | **ARQUITECTURA 2-COLUMNAS:** (1) **Sidebar Izquierdo (250px fijo):** Título "Últimos Reportes Enviados". Contenedor con máx. 10 tarjetas `<mat-card>` históricas (auto-purga, más antiguo desaparece al agregar #11). Cada tarjeta: badge circular rojo "R" o azul "B" (size 48px, font-weight bold), título "Reporte [R]" o "Boletín [B]", timestamp gris "Fecha - Hora", dos botones de acción alineados (mat-icon-button): icon "download" para descargar PNG, icon "open_in_new" para abrir en pestaña nueva. Pie: botón "Cargar Más" (mat-stroked-button) deshabilitado/grayed cuando historial ≥ 10 eventos con tooltip "Límite de historial alcanzado (10 eventos máximo). Los más antiguos se eliminan automáticamente." (2) **Área Central (flex grow):** Header con título "Generador de Reportes SOC", subtítulo "Completa la información para generar tabla HTML", botón outline "Guía Rápida" top-right (mat-stroked-button, pequeño). Botones toggle horizontal centrados: "Reporte de Incidente" | "Boletín de Seguridad" (mat-button-toggle-group). **FORMULARIO GRID:** Fila 1: input full-width label "Título del Boletín / Amenaza". Fila 2: grid responsive 3-col: input "Marca / Fabricante", input "CVE / Identificadores", select "Nivel de Alerta (Criticidad)" con opciones "Bajo"✓green-dot, "Medio"✓orange-dot, "Alto"✓red-dot. Filas 3-5: textarea grande (multiline) label "Producto(s) Afectado(s)", "Impacto", "Acciones Recomendadas / Mitigación". Fila 6: input full-width label "Enlaces de Referencia". **VISTA PREVIA RÁPIDA:** Bloque below form, label "Vista Previa Rápida" con placeholder skeleton (4 líneas gris claro), preview live actualiza en tiempo real del template final (rojo header banner para Reporte, tabla limpia datos para Boletín). **ACCIONES FOOTER:** Alineación derecha, gap 12px: botón secondary "Previsualizar" (mat-stroked-button), botón primary "Generar Reporte HTML" (mat-raised-button color=primary, loading spinner dentro cuando estado isGenerating=true). **NOTIFICACIÓN ÉXITO:** Toast mat-snack-bar top-right (auto-hide 4s): "Reporte creado. Imágenes PNG de respaldo generadas y reporte enviado por correo." Tema light: fondo blanco/gris 50, tipografía Roboto 14px body, bordes #e0e0e0 1px subtle, card hover sombra leve. **ARCHIVOS:** `report-generator.component.ts` (lógica toggle form labels, form group ReactiveForm, descarga PNG, historial 10-max con purga, isGenerating spinner), `report-generator.component.html` (layout sidebar+main, form grid responsive, preview placeholder, button loading state), `report-generator.component.scss` (sidebar 250px fixed, main flex-grow, media <768px sidebar collapse vertical, grid gap 12px, card hover box-shadow, input focus border-color primary). **CRITERIOS VALIDACIÓN:** (1) Layout responsive 2-col desktop, sidebar stacked mobile <768px. (2) Toggle Reporte↔Boletín cambia dinámicamente form labels, select options y preview template. (3) Historial limita máximo 10, auto-purga cuando se agrega #11, botón "Cargar Más" disabled con tooltip visible. (4) Botones acción descarga PNG (download) y open-external funcionales. (5) Form completo sin errores compilación, validación input required/pattern. (6) Toast notificación aparece on-success create. (7) Vista previa actualiza en vivo keystroke, placeholder visible mientras carga. (8) Button "Generar" muestra spinner loading, deshabilita click durante generación. (9) Responsive test: Tab 768px breakpoint, Mobile 360px breakpoint, Desktop 1920px. |
