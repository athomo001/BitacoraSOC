...
| ID | Estado | Seccion | Tarea | Notas |
| :--- | :--- | :--- | :--- | :--- |
| UI-REPORT-GEN-140 | Listo | UX/UI + Frontend CRÍTICA | Reinventar UI del "Generador de Reportes SOC" con nuevo diseño moderno | Se implementó el diseño de 2 columnas (sidebar izquierdo para 10 reportes/boletines históricos auto-purgados, botones de descarga PNG y abrir, botón 'Cargar Más' con tooltip de límite; y área central flexible con toggles de reporte/boletín, guía rápida, formulario grid y panel de detalles adicionales del incidente). También se agregó la vista previa rápida en vivo con skeleton de 4 líneas y las acciones del footer con inline spinner para `isGenerating`. La notificación de éxito de 4s indica el envío por correo. Validado mediante compilación Docker exitosa. |
| UI-LOAD-INCON-101 | Listo | UX/UI + Interacción MEDIA | Consistencia en estados de carga | Se agregó `mat-progress-bar mode="indeterminate"` en `audit-logs.component.html` (antes de tabla) con propiedad `isLoading` controlada en `loadLogs()`. Se agregó en `report-generator.component.html` con propiedad `isGenerating` controlada en `generateTable()`. Ahora todas las vistas muestran barra de progreso durante cargas. |
| UI-H1-CHECK-102 | Listo | UX/UI + Accesibilidad MEDIA | Jerarquía de headings en checklist.component | Se agregó `<header class="page-header"><h1>Checklist de Turnos</h1></header>` al inicio de `checklist.component.html`. Estructura semántica completa con heading level 1 para el contenedor principal. |
| UI-DIV-AUDIT-103 | Listo | UX/UI + Semántica MEDIA | Uso de <div> en audit-logs.component | Se convirtieron todos los paneles a `<section>`: Guía (`<section>` + `<header><h2>ℹ️ Guía`), Filtros (`<section>` ya tenía header), Resultados (`<section>` ya tenía header). Estructura semántica completa. |
| UI-DIV-ACTIONS-105 | Listo | UX/UI + Semántica BAJA | Uso de clase .actions en reports | Se cambió `<div class="actions">` a `<div class="form-actions">` en `reports.component.html` línea 479 para consistencia con design system. |
| UI-SKEL-106 | Pendiente | UX/UI + Performance BAJA | Ausencia de skeleton loading | Skeleton implementado en checklist y my-entries. Falta en: `audit-logs.component.html` (tabla mat-table) y `report-generator.component.html` (tabla resultados). |
| SHIFT-TI-SEL-136 | Listo | Admin / Turnos MEDIA | Selector de Especialista TI cargando directorio centralizado | Se corrigió el selector de rol `TI` para cargar 20+ contactos internos desde el directorio centralizado en lugar de limitarse a 3 usuarios del sistema. Muestra formato `Nombre Completo • Teléfono` con clase CSS `.option-phone`. Se modificó `saveWeeklyAssignment()` para permitir asignación directa desde directorio interno sin buscar coincidencia en tabla de usuarios del sistema. |
| SHIFT-PROX-137 | Listo | Admin / Turnos MEDIA | Cálculo de progreso en "Próximos Turnos a Iniciar" | Se implementó cálculo dinámico de barra de progreso para la sección "Próximos Turnos a Iniciar" utilizando fórmula `progress = (tiempo restante) / (duración total) × 100`. Anteriormente estaban hardcodeadas a 0%. Ambas secciones ahora aplican el mismo sistema de colores: verde >80%, amarillo 50-80%, rojo <50%. |
| UI-SEM-091 | Listo | UX/UI + Frontend ALTA | Limpieza de Div-soup y semántica HTML | Limpieza completa: `reports.component.html` (stat-cards) migradas de `<div>/<p>` a `<dl><dt><dd>` con reorden semántico (label primero, luego valor). `users.component.html` convertidas ambas secciones "Crear/Editar" y "Usuarios Registrados" de `<div class="users-panel">` a `<section>` con `<header><h2>` semántico. Escalación completada en sesión anterior. |
| UI-CONFIRM-DEL-108 | Listo | UX/UI + Interacción ALTA | Migración completa de window.confirm() a MatDialog | Se reemplazó `window.confirm()` en `admin-complements.component.ts` línea 1112 por `MatDialog` con confirmación estilizada. Creado `ConfirmDeleteComponent` standalone con botones "Cancelar" y "Eliminar" (color warn). MatDialog ya implementado en todos los módulos (escalation, catalog, work-shifts, etc.). |
| CAT-INT-115 | Listo | Admin / Catálogos ALTA | Restringir check de Cliente Interno a un único cliente | En `/main/admin/catalogs`, permitir solo **1** cliente con `cliente interno` activo a la vez. Si se quiere marcar otro, primero se debe desmarcar el actual. Validar en UI y backend para evitar estados inválidos por concurrencia. |
| CAT-INT-116 | Listo | Admin / Catálogos ALTA | Correo obligatorio para Cliente Interno (CC global) | Al activar `cliente interno`, mostrar panel/campo para registrar correo o lista de correos (formato email válido). Ese correo quedará como CC automático en escalaciones. El panel solo aparece cuando el check está activo y respeta la restricción de cliente interno único. |
| CAT-UX-117 | Listo | Admin / Catálogos MEDIA | Unificar nomenclatura: quitar "Log Source" y usar "Cliente" | En `/main/admin/catalogs`, reemplazar textos/labels donde aparezca "log source" por "cliente" para consistencia funcional y semántica del módulo. |
| CAT-UX-118 | Listo | Admin / Catálogos MEDIA | Diferenciación visual del Cliente Interno en tabla de clientes | En la tabla de clientes, destacar visualmente el cliente interno (badge/etiqueta/estilo) y mostrar el correo configurado para CC global, de forma legible en todos los temas. |
| ESC-UX-119 | Listo | Admin / Escalación ALTA | Simplificar selector de cliente en Escalación (quitar duplicidad buscador+combo) | En `/main/admin/escalation`, eliminar duplicidad de controles de selección de cliente (buscador + combo redundante). Dejar una sola interacción con autocompletado que permita escribir y sugerir clientes similares para elegir. |
| ESC-DATA-120 | Listo | Admin / Escalación ALTA | Carga automática de CC desde Cliente Interno y autocompletado de contactos | En escalaciones, el campo CC debe autocompletarse con el correo del cliente interno configurado en catálogos. En la vista operativa, mostrar solo el nombre del cliente seleccionado y cargar automáticamente los contactos desde el directorio centralizado, validando que no falten registros. |
| ESC-LAYOUT-121 | Listo | Admin / Escalación MEDIA | Optimización visual y espacio en paneles de Escalación | Ajustar "Servicios de Escalación" para altura contenida con scroll cuando corresponda. Eliminar completamente bloque "Clientes (fuente única)" para ganar espacio sin perder funcionalidad. |
| ESC-TABS-122 | Listo | Admin / Escalación MEDIA | Reordenar y renombrar pestañas de Escalación | Orden final: `Contactos y Agenda`, `Flujo de Escalamiento`, `Matriz RACI`, `Turnos Internos`. Renombrar etiquetas visibles a: `Flujo de Correos`, `Flujo de llamadas`, `Matriz RACI`. Retirar `Turnos Internos` de este módulo (migración a `/main/admin/work-shifts`). |
| ESC-FLOW-123 | Listo | Admin / Escalación ALTA | Mejoras UX y estado de configuración en Flujo de llamadas | En `Flujo de llamadas/Flujo de Escalamiento`, eliminar campos redundantes tipo cajón de texto y mostrar visualmente qué clientes están configurados/no configurados. Incluir resumen visual del estado actual para evitar validación manual constante en otras vistas. |
| SHIFT-MOVE-124 | Listo | Admin / Turnos ALTA | Migrar "Asignar Turnos Semanales" a `/main/admin/work-shifts` | Mover completamente la funcionalidad de turnos semanales fuera de `/main/admin/escalation` hacia `/main/admin/work-shifts`, preservando compatibilidad de datos, navegación y comportamiento para no romper flujos existentes. |
| SHIFT-UX-125 | Listo | Admin / Turnos ALTA | Rediseñar `/main/admin/work-shifts` con layout dashboard de 2 columnas + sidebar | Implementar estructura visual tipo "Panel de Gestión de Turnos y Roles": fondo limpio, tarjetas claras, jerarquía visual consistente y responsive desktop/móvil (bloque central + editor lateral). |
| SHIFT-GANTT-126 | Listo | Admin / Turnos ALTA | Vista Resumen Semanal con timeline tipo Gantt y línea de Día Actual | En la tarjeta "Vista Resumen Semanal", agregar eje temporal semanal, barras por persona/rol y una línea vertical destacada para `Día Actual` con fecha visible. Estado de barras: `En Curso`, `Próximo` y `Pasado` con colores semánticos. |
| SHIFT-PROX-127 | Listo | Admin / Turnos ALTA | Tarjetas de turnos por proximidad (terminar/iniciar) con orden estricto | Crear tarjetas "Turnos Próximos a Terminar" y "Próximos Turnos a Iniciar" ordenadas por cercanía real a la fecha actual. Cada fila debe incluir persona, rol, texto de tiempo restante/inicio y barra de progreso compacta. |
| SHIFT-FORMAT-128 | Listo | Admin / Turnos MEDIA | Bloque "Formato de Datos de Asignación" contextual y visible | Incorporar tarjeta informativa con formato esperado de datos (`roleCode, userType, weekStartDate, weekStartTime, weekEndDate, weekEndTime, notes`) para reducir errores de carga manual. |
| SHIFT-EDITOR-129 | Listo | Admin / Turnos ALTA | Editor de Asignación lateral con validaciones de solape y disponibilidad | En el panel "Editor de Asignación", incluir selectores de `Rol`, `Persona` y `Semanas Disponibles` (inicio/fin). Validar traslapes de turnos, rango inválido y disponibilidad real antes de guardar. |
| SHIFT-SAVE-130 | Listo | Admin / Turnos MEDIA | Acción "Guardar Asignación" con feedback claro de éxito/error | Botón primario visible con estados `loading/success/error`, mensajes accionables y refresco inmediato de resumen/tabla al persistir cambios. |
| SHIFT-FILTER-131 | Listo | Admin / Turnos MEDIA | Filtros rápidos por persona y rol en gestión de asignaciones | Añadir barra de búsqueda/filtro en el bloque de gestión para filtrar por `Persona` y `Rol` sin recargar toda la vista, manteniendo consistencia con controles existentes. |
| SHIFT-TABLE-132 | Listo | Admin / Turnos ALTA | Tabla detallada de asignaciones con estados y acciones | En "Gestión de Asignaciones", asegurar columnas: `Estado`, `Semana`, `Rol`, `Persona`, `Comentarios/Notas`, `Acciones`; incluir pills de estado (`En Curso`, `Próximo`, `Pasado`) y acciones de edición/eliminación con iconografía consistente. |
| SHIFT-DATA-133 | Listo | Admin / Turnos ALTA | Consistencia de datos entre resumen, tarjetas y tabla | Garantizar que los datos de timeline, tarjetas de proximidad y tabla usen la misma fuente y reglas de cálculo para evitar discrepancias visuales/operativas. |
| SHIFT-I18N-134 | Listo | Admin / Turnos MEDIA | Texto completamente en español en dashboard de turnos | Normalizar labels, títulos, ayudas, estados y mensajes de validación al español en toda la vista de turnos, evitando mezclas con inglés en UI final. |
| SHIFT-NAV-135 | Listo | Admin / Turnos BAJA | Pulir integración visual con navegación lateral y topbar global | Revisar que la vista de turnos respetete el shell existente (menú lateral, iconos superiores, espaciado y alineación) sin romper patrón visual del layout principal. |
| SHIFT-TI-SEL-136 | Listo | Admin / Turnos MEDIA | Selector de Especialista TI cargando directorio centralizado | Se corrigió el selector de rol `TI` para cargar 20+ contactos internos desde el directorio centralizado en lugar de limitarse a 3 usuarios del sistema. Muestra formato `Nombre Completo • Teléfono` con clase CSS `.option-phone`. Se modificó `saveWeeklyAssignment()` para permitir asignación directa desde directorio interno sin buscar coincidencia en tabla de usuarios del sistema. |
| SHIFT-PROX-137 | Listo | Admin / Turnos MEDIA | Cálculo de progreso en "Próximos Turnos a Iniciar" | Se implementó cálculo dinámico de barra de progreso para la sección "Próximos Turnos a Iniciar" utilizando fórmula `progress = (tiempo restante) / (duración total) × 100`. Anteriormente estaban hardcodeadas a 0%. Ambas secciones ahora aplican el mismo sistema de colores: verde >80%, amarillo 50-80%, rojo <50%. |
