...
| CAT-NOMEN-REORG-144 | Listo | Admin / Catálogos MEDIA | Renombrar y reorganizar pestañas en "Administración de Clientes y Catálogos" | Implementado en `/main/admin/catalogs`: título actualizado a "Administración de Clientes y Catálogos", renombre de navegación a "Clientes y Catálogos" y reorden de tabs a `Clientes`, `Tipos de Operación`, `Alertas y Mantenimientos`, `Tipo de Eventos` (renombrada desde `Eventos`). Validado con build frontend exitoso y reflejado en `docs/CHANGELOG.md`. |
