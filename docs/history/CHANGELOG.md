...
- **Docs — `spec/00-mapa-mental.md` y `spec/README.md` actualizados** reflejando las 4 decisiones cerradas; checklist de salida de la Fase 1 en `spec/02-alcance-y-roadmap.md` marcado como cumplido. Fase 2 (Infraestructura Base) queda desbloqueada.

---

## [Rewrite] Fase 2 — Infraestructura Base — CERRADA - 2026-09-22

- **Backend — Repositorio Go inicial** (`backend-go/`, ver `docs/adr/0012-repo-monolito-carpetas-nuevas.md`): módulo `github.com/athomo001/BitacoraSOC/backend-go` en Go 1.27, estructura `cmd/server/`, `internal/{repository,handler}/`, `sql/{schema,migrations,queries}/` según `spec/07-backend-arquitectura-go.md` sección 1. `go build`/`go vet` en verde.
- **Fix — 2 bugs reales en `spec/03-esquema-db.sql` que rompían la ejecución del DDL de corrido**, encontrados al aplicarlo por primera vez contra un Postgres 18 real: la tabla `tickets` declaraba FK inline hacia `assets`/`teams`/`contacts` (definidas más abajo en el archivo — Postgres no permite forward-reference en `REFERENCES` inline) y usaba el tipo `entry_scope` antes de su `CREATE TYPE`. Corregido con el mismo patrón de FK diferida (`ALTER TABLE ... ADD CONSTRAINT`) que el archivo ya usaba en otros 2 lugares, y adelantando el `CREATE TYPE entry_scope`. Verificado end-to-end: el DDL corre sin errores y crea 52 tablas + 1 vista. El conteo "~46 tablas" citado en 3 lugares de la spec (nunca antes verificado contra una corrida real) se corrigió a "~52".
- **Infra — `docker-compose.rewrite.yml`** (raíz del repo, separado del `docker-compose.yml` de producción legacy): 3 servicios — `bitacora-db` (Postgres 18 + `ltree`), `bitacora-app` (binario Go, sin puerto publicado al host), `caddy` (único punto de entrada, `127.0.0.1:8081`, `Caddyfile.rewrite`). Bug real encontrado al levantarlo por primera vez: la imagen `postgres:18` cambió su convención de volumen de datos (monta en `/var/lib/postgresql`, ya no en `/var/lib/postgresql/data`); corregido y documentado en el propio compose. Segundo bug real al agregar Caddy: sin `flush_interval -1` en el `reverse_proxy`, Caddy bufferea la respuesta y el streaming SSE en vivo queda completamente bloqueado — corregido y documentado en `Caddyfile.rewrite`.
- **Backend — Migraciones y capa de datos reales**: `sql/migrations/000001_init_schema.{up,down}.sql` aplicadas con el binario real de `golang-migrate` contra el contenedor (`migrate up` verificado, 53 tablas incluyendo `schema_migrations`); `sqlc` (`v1.31.1`) generando código tipado desde `sql/schema`/`sql/queries` sobre `pgx/v5`.
- **Backend — `GET /api/health/live` y `GET /api/health/ready`**: `ready` ejecuta `Queries.Ping()` (SQL real vía sqlc, no un simple TCP connect) con timeout de 2s. Verificado con `curl` a través de Caddy contra el contenedor real: `200 {"status":"ready"}`.
- **Backend — `internal/crypto`** (AES-256-GCM, desarrollado con TDD — round-trip, llave incorrecta, ciphertext alterado, nonce distinto por llamada): cifrado/descifrado explícito de secretos en reposo, reemplazando una referencia rota en la spec a un patrón `EncryptedString`/`Scan`/`Value` que nunca se había definido en ningún lado.
- **Backend — `internal/service/mail`**: envío SMTP real (`net/smtp`), verificado con un test de integración contra un catcher SMTP real (Mailpit) confirmando entrega leyendo su API HTTP. Endpoints HTTP (`GET/PUT /api/config/smtp`, `POST /api/config/smtp/test-send`) agregados a `spec/04-contratos-api.md` (gap real, no existían) y quedan para la Fase 4 (necesitan auth real).
- **Backend — `internal/eventbus`** (hub SSE genérico, TDD) + **`GET /api/stream/events`**: verificado de punta a punta contra el contenedor real — reposición vía `Last-Event-ID` tras un `docker restart bitacora-app` sin perder los eventos publicados antes/después del reinicio, y entrega en vivo de un evento publicado por el proceso mismo. Límite arquitectónico real documentado en `spec/09-alta-disponibilidad-2-nodos.md`: el hub es por-proceso, un evento insertado por otro nodo/proceso no llega en vivo, solo al reconectar.
- **CI — `.github/workflows/backend-go-ci.yml`**: build + vet + gofmt + test para `backend-go/`, activado por cambios en esa carpeta (corre localmente, no pusheado a GitHub todavía).
- **Cierre de fase**: único punto no resuelto (`stylelint`) es estructuralmente imposible antes de que exista `frontend-v2/` (Fase 3) — aceptado explícitamente como excepción, no como deuda escondida. Checklist de salida completo en `spec/02-alcance-y-roadmap.md`. Fase 3 (Frontend Shell y Design System) queda desbloqueada.
...
- **Feature (Backend/Frontend) — Vincular manualmente una entrada a un ticket GLPI existente al crearla** (`/main/entries`):
  - Nuevo campo opcional "Ticket GLPI" junto a "Hora del Evento" en el formulario de Nueva Entrada; si se completa, al subir la entrada se vincula automáticamente al ticket indicado y se envía su contenido como seguimiento (mismo mecanismo que "Vincular a ticket GLPI" en Todas las Entradas), sin bloquear la creación de la entrada si la vinculación falla (queda un aviso aparte).
  - Nuevo toggle de administrador **"Mostrar campo 'Ticket GLPI' en Nueva Entrada"** (`/main/admin/integrations` → GLPI), controlado por un campo nuevo `manualLinkFieldEnabled` en `GlpiConfig`, expuesto vía un endpoint público liviano (`GET /api/integrations/glpi/manual-link-field`, solo requiere sesión autenticada, no rol admin) para que cualquier analista pueda ver el campo sin exponer el resto de la configuración GLPI.
  - Este toggle es **independiente** del interruptor maestro "Habilitar GLPI": un admin puede activar el campo de vinculación manual sin encender el resto de la integración (despacho automático, importación entrante, etc.).
  - Si la entrada es de tipo `incidente`/`ofensa` y ya se indicó un ticket manualmente, se omite la creación automática de un ticket nuevo para evitar duplicados.
- **Bugfix (Backend/Frontend) — El panel de GLPI exigía Base URL/App-Token/User Token para guardar cualquier cambio, aunque "Habilitar GLPI" estuviera apagado**: tanto la validación del formulario (frontend) como una validación independiente en el backend (`applyGlpiConfigPayload`) exigían los campos de conexión sin considerar si la integración estaba realmente habilitada, impidiendo guardar ajustes no relacionados (como el toggle de vinculación manual) sin completar antes toda la conexión GLPI. Ambas validaciones ahora solo aplican cuando `enabled=true`.
- **Feature (Backend/Frontend) — Correo del área en copia (CC) en las felicitaciones de cumpleaños** (`/main/admin/users`):
  - Nuevo campo "Correo del área (CC)" en la tarjeta "Automatización de Cumpleaños", con autocompletado en vivo contra el Directorio de Contactos (o texto libre si el correo no está en el directorio).
  - Nuevo campo `birthdayEmailsCcAddress` en `AppConfig`; el correo de cumpleaños se sigue enviando siempre a quien cumple años, y si hay un correo de área configurado, se agrega en copia (CC) para que el área sepa que se envió.
  - Se rediseñó la tarjeta para que sea visualmente más compacta y el mensaje descriptivo se acortó.
- **Bugfix (Frontend) — El menú lateral principal no colapsaba en pantallas angostas, afectando toda la aplicación**: el `mat-sidenav` usaba `mode="side"` fijo (280px, siempre empujando el contenido) sin ninguna lógica responsive, por lo que en ventanas/celulares angostos el contenido quedaba con tan poco espacio horizontal que el texto se veía cortado en vez de envolver, en cualquier pantalla del sistema (no solo administración). Se agregó `BreakpointObserver` (`@angular/cdk/layout`) para que, por debajo de 960px de ancho, el drawer pase a modo overlay (`mode="over"`), arranque cerrado y se cierre automáticamente al navegar a otra sección.
- **Mejora (Frontend) — Rediseño de la Consola Administrativa** (`/main/admin`): se elimina el banner de título "Consola Administrativa" + subtítulo y la fila de botones de navegación independiente; se reemplazan por una barra de pestañas Angular Material con ícono + texto por sección (mismo patrón ya usado en `/main/escalation/view`), con scroll horizontal cuando no caben todas, ahorrando espacio vertical.
- **Bugfix (Frontend) — ID de Mongo crudo visible en la tabla de Recordatorios de Checklist** (`/main/admin/checklist`): cuando un recordatorio periódico de turno referenciaba un turno que ya no está activo (desactivado o eliminado), la columna "Turnos" mostraba el `ObjectId` crudo en vez de un texto legible (`shiftNameById` caía a `?? id`). Ahora muestra "Turno inactivo".
- **Bugfix (Frontend) — Auditoría de consistencia visual en 9 páginas de administración** (Checklist, Usuarios, Seguridad, EMAIL Config/SMTP, Clientes y Catálogos, Escalación —4 pestañas—, Integraciones, Apariencia, Complementos):
  - **Checkboxes desalineados de su campo relacionado**: en varios formularios, un `mat-checkbox` que habilita un campo dependiente (ej. "Avisar checklist no realizado" + "Hora límite"; SSO Google/Microsoft + Client ID) estaba apilado como bloque suelto en vez de ir en la misma fila centrado contra la altura del campo, o el campo dependiente se mostraba siempre visible en vez de solo cuando el checkbox está activo. Se agruparon en filas (`align-items: center`) y se aplicó revelado condicional (`*ngIf`) donde correspondía (SSO Google/Microsoft en Seguridad, checkboxes sueltos dentro del grid de campos en SMTP movidos a un grupo aparte).
  - **Botones de guardar sin envoltorio de acciones consistente**: varios formularios tenían su botón principal suelto directamente en el `<form>`, con `style=""` en línea para separarlo del contenido de arriba, en vez de usar la clase compartida `.form-actions`. Corregido en Seguridad (SSO), EMAIL Config (modo invitado) y Checklist (con ícono + spinner de guardado).
  - **Doble borde "caja dentro de caja"**: la barra de navegación rápida del asistente de Checklist ("1. Configuración / 2. Recordatorios / 3. Plantillas") tenía su propio borde y fondo alrededor de botones que ya traen su propio borde (`mat-stroked-button`); se quitó el borde/fondo del contenedor.
  - **Banner de advertencia con colores hexadecimales fijos**: en Directorio de Contactos, el aviso "Contacto sincronizado desde Usuarios" tenía una clase CSS incompleta (solo layout, sin color) parchada con `style="background-color:#fff3cd..."` en línea — no se adaptaba a los demás temas de la aplicación. Se completó la clase con los tokens de tema (`--state-warning`/`--state-warning-bg`) y se quitaron los estilos en línea (junto con otros 3 casos menores en el mismo archivo).
  - **Espaciados hardcodeados en vez de tokens del sistema de diseño**: ~130 valores de `gap`/`margin`/`padding` en px repartidos en 8 componentes usaban números sueltos (`8px`, `12px`, `16px`...) en vez de las variables `var(--space-1)` a `var(--space-6)` ya definidas en el proyecto, generando un ritmo de espaciado inconsistente entre pantallas. Normalizados a tokens (dejando intencionalmente sin tocar el padding interno de badges/chips y ajustes de alineación fina, que no son espaciado de layout).
  - **Título de pestaña con tamaño distinto al resto**: en Escalación, la pestaña "Matriz RACI" tenía `font-size: 1.8rem` en su título mientras las otras 3 pestañas (Flujo de llamadas, Flujo de Correos, Directorio) usaban `1.4rem`. Igualado a `1.4rem`.
  - **Texto pegado al borde del panel**: 2 de las 4 pestañas de Escalación (Flujo de llamadas, Flujo de Correos) tenían `padding-top` únicamente en su contenedor, sin margen lateral, mientras las otras 2 sí lo tenían. Igualado el padding en las 4.
  - **Columnas de altura desigual en Usuarios**: la tarjeta "Crear Nuevo Usuario" terminaba mucho antes que la columna lateral (dos tarjetas apiladas), dejando un hueco de fondo gris antes de la tabla de usuarios. Se cambió `align-items: start` a `stretch` en el grid de dos columnas para que ambas igualen su altura.
  - **Checkbox de MFA con HTML crudo y estilos en línea**: el checkbox "Habilitar Autenticación Multifactor (MFA/TOTP)" en Crear/Editar Usuario era un `<input type="checkbox">` nativo con `style=""` en vez de `<mat-checkbox>`, visualmente distinto a cualquier otro checkbox de la aplicación. Convertido a `mat-checkbox`.
- **Bugfix (Frontend) — Asterisco duplicado en 21 campos obligatorios** (Clientes y Catálogos ×6, Generador de Reportes ×10, Matriz RACI ×2, Checklist ×2, Directorio de Contactos ×1): Angular Material ya agrega automáticamente un asterisco al `mat-label` de todo campo con `Validators.required` (o atributo `required`); varias plantillas además escribían un `*` a mano en el texto del label, mostrando `Nombre **`. Se quitó el asterisco manual, dejando solo el automático de Material.
- **Docs — `AGENTS.md` actualizado a la realidad del proyecto**: la sección de documentación obligatoria apuntaba a una carpeta `spec/` que no existe en el repositorio; se corrigió para apuntar a `docs/` (arquitectura, despliegue, operaciones, API, módulos extra, seguridad, MongoDB). La sección de skills pasó de una mención genérica a un catálogo real de las 38 skills disponibles en `.agents/skills/`, agrupadas por uso.
...
## [Rewrite] Fase 8 — Turnos, Rotación y Dotación — CERRADA - 2026-09-23

- **Backend — motor de rotación de guardia** (`internal/rotation`, TDD, mismo patrón que `internal/escalation`): resuelve quién está de guardia ahora — un reemplazo puntual vigente gana siempre sobre el rol semanal regular; una fila del rol marcada como pausada se excluye sin buscar un reemplazo automático (pausar y dejar el hueco a la vista es la idea, no taparlo solo).
- **Gap real de contrato encontrado y cerrado** (mismo patrón que las Fases 5-7): el contrato original no traía forma de armar ni pausar el rol semanal (`rotation_slots`) — se agregaron `GET/POST/PATCH /api/rotation-slots`. Se revisó el legacy antes de diseñar: confirma que ese rol siempre se armó a mano, semana por semana, nunca por generación automática. Se dejó sin `DELETE` a propósito — pausar conserva el historial en vez de borrar, a diferencia del legacy.
- **Backend — matriz semanal de dotación/teletrabajo** (`GET /api/work-shifts/matrix`): Lunes a Viernes, una fila por analista interno, con quién tiene una novedad esa semana (licencia, vacaciones, trámite médico, capacitación, guardia, teletrabajo) listado primero. Colores tomados de la misma paleta semáforo del resto de la app, no de la paleta del legacy.
- **Backend — enlace público de solo lectura para la pantalla/TV de sala** (`POST /api/public-shares/telework`, `GET /p/telework/{token}`): primera página HTML servida directo por el backend, sin sesión. El token se genera al azar y solo se guarda su huella (nunca el token en claro, a diferencia del legacy); se autorrefresca cada 10 minutos y se actualiza sola si se regenera o desactiva.
- **Backend — notificación periódica de dotación a RRHH** (HU-5b): la programación (a quién, cuándo, qué filtro de rol) queda completa, pero el envío automático real todavía no tiene quién lo dispare — no hay ningún cron en el backend todavía, eso llega recién con la Fase 12 ("Notificaciones Programadas y Pulido de Correo"). El cuerpo del correo (texto plano) se probó a mano contra un servidor de correo de prueba.
- **Cierre de un pendiente de la Fase 7**: en la pantalla de Escalamiento, quien está de guardia ahora ya se distingue y se llama primero — antes ese dato siempre venía vacío porque el motor de rotación no existía todavía.
- **Frontend — pestaña Dotación** (`/shifts`): la grilla semanal, navegación entre semanas, y (solo para administradores) edición de un día con un clic y gestión del enlace de TV. La otra pestaña del mismo lugar ("Mi Turno", el checklist de inicio/cierre) sigue pendiente, llega en la Fase 11.
- **Frontend — Administración → Turnos**: dar de alta el ciclo de guardia de un equipo, armar y pausar su rol semanal, cargar reemplazos puntuales, definir turnos y configurar el correo periódico de dotación.
- **Verificado**: la lógica (Go y Angular) tiene pruebas automáticas y pasan todas, y ambos proyectos compilan limpios. Además, ya con Docker disponible, se probó contra el sistema real de punta a punta: se armó un equipo de guardia de prueba con su ciclo y rol semanal, se confirmó que un reemplazo puntual le gana al rol regular, y que ese reemplazo se refleja de verdad en la pantalla de Escalamiento (la persona de guardia sale primero, aunque su rol documental sea "respaldo"). También se generó el enlace de TV de verdad (se ve sin sesión, se auto-refresca, y revocarlo lo corta al instante) y se mandó un correo real de prueba de la notificación de dotación contra un servidor de correo de prueba, confirmando la entrega.

---

## [Rewrite] Fase 9 — Bitácora — CERRADA - 2026-09-24

- **Backend — registro operativo central** (`entries`/`entry_comments`): crear, listar con filtros (ámbito, tipo, tag, fechas, texto libre), ver el detalle con sus comentarios, editar, borrar y reclasificar en lote. Alcance recortado a lo que pedía esta fase: sin la parte que vincula con la ticketera (llega en la Fase 10) ni con GLPI (quedó fuera del corte desde la Fase 1).
- **Bug real encontrado, arrastrado desde hace varias fases**: en todo el spec, la búsqueda de texto libre decía apoyarse en una función de Postgres llamada `websearch_to_query` — ese nombre nunca existió; la función real se llama `websearch_to_tsquery`. Nadie lo había notado porque ninguna fase anterior había ejecutado esa búsqueda contra una base de datos real. Corregido en el spec y en el código; probado además con el caso que motivaba usar esa función en primer lugar: buscar algo como `"firewall &"` ya no rompe nada.
- **Decisión tomada con el dueño esta sesión**: la imagen de una entrada se sube y se guarda tal cual, sin convertirla a un formato más liviano ni limpiarle los datos ocultos de la cámara/dispositivo que la tomó. Esto ya estaba decidido así en la documentación interna del proyecto, pero dos otros documentos se habían quedado desactualizados y decían lo contrario — se dejaron todos consistentes. La razón de fondo: hacer esa conversión bien exige herramientas que no son compatibles con la forma simple y liviana en que está armado el resto del sistema, o herramientas nuevas sin trayectoria comprobada.
- **Gap real encontrado**: la imagen se sube antes de guardar la entrada (para poder mostrarla en la vista previa mientras se escribe), pero la tabla donde vive esa imagen exigía que la entrada ya existiera. Se ajustó para permitir una imagen "en espera" que se asocia a la entrada recién en el momento de guardarla — sin que en ningún momento la imagen toque el disco del servidor (todo va a la base de datos, un requisito ya existente pensado para cuando este sistema corra en dos servidores en paralelo).
- **Frontend — primera conexión en vivo del sistema**: el aviso de "hay una versión nueva disponible" ahora llega de verdad en tiempo real a la pantalla, sin que el usuario tenga que recargar para enterarse.
- **Frontend — autoguardado**: lo que se está escribiendo en el cajón de redacción se respalda solo cada 10 segundos, y las dos libretas de notas (el pizarrón compartido y la personal) se guardan solas 3 segundos después de dejar de escribir.
- **Bug real encontrado recién al probar con Angular de verdad** (no aparecía probando la API sola): la pantalla de Bitácora se rompía al cargar por un error de "fuera de contexto" en un componente interno de Angular. Corregido.
- **Frontend — pantalla de Bitácora** (`/entries`): muro con filtros, detalle de cada entrada con sus comentarios, redacción rápida con pegado de imagen (Ctrl+V), y los paneles de pizarrón/libreta personal en la misma pantalla.
- **Verificado**: contra el sistema real completo (API y base de datos reales) — búsqueda por tag y por texto (incluida la búsqueda "rota" que ahora no rompe nada), comentarios, borrado (una entrada ajena da error, la propia se borra), edición en lote, imagen subida y recuperada, borradores guardados y recuperados, notas guardadas, y el aviso en vivo llegando de verdad. También se probó en un navegador real: iniciar sesión, ver el muro con datos reales, abrir una entrada y ver su imagen y comentarios, escribir y guardar una entrada nueva y verla aparecer al toque, y ver el contenido real de las notas — todo sin errores.
