...
| :--- | :--- | :--- | :--- |
| **Objetivo Principal** | Continuidad operacional por turno: qué pasó, quién lo tomó, qué quedó pendiente y a quién escalar. | Gestión de incidentes de ciberseguridad e investigación técnica profunda (IOC, forense, respuesta). | Gestión de solicitudes/incidentes de servicio con trazabilidad administrativa y SLA. |
| **Centro de Gravedad** | **Operación en tiempo real y relevo de guardia** (SOC, NOC, TI on-call y equipos de emergencia). | **Investigación de amenazas** y respuesta técnica especializada orientada a la la Ciberseguridad y no al enfoque General. | **Proceso de atención** (tickets, colas, aprobaciones, catálogo de servicios). |
| **Operación** |  **Global y General**, se puede utilizr en sectones **no necesariamente de ciberseguridad** | Ciberseguridad |
...
| **Enfoque de Escalación** | **Humano-organizacional + reglas** (directorio, RACI, contactos por cliente/servicio y envío programado de turnos). | **Técnico-automatizado** (playbooks y enriquecimiento de IOCs vía Cortex). | **Jerárquico por flujo de atención** (grupos, colas, aprobaciones y notificaciones). |
...
| **Modelo de Datos Operativo** | **Evento + narrativa + checklist + auditoría** (prioriza contexto de turno y continuidad entre personas). | **Caso + observable + tarea** (prioriza investigación y evidencia técnica). | **Ticket + estado + SLA** (prioriza ciclo de vida de atención). |
...
| **Reportes y Comunicación** | Boletines/reportes ejecutivos con envío por correo y opción de agrupar destinatarios por dominio. | Requiere capas adicionales para reporteo ejecutivo orientado a cliente. | Muy fuerte en reportes de servicio/SLA; menos enfocado en narrativa de relevo de guardia. |
